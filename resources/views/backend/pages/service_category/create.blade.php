...
@section('content')
  @component('backend.common-components.breadcrumb')
    ...
    @endslot
    @slot('title')
      {{ isset($edit_data) ? 'Services Category Edit' : 'Services Category Create' }}
    @endslot
  @endcomponent
        ...
        <div class="card">
          <div class="card-body">
            ...
            <form method="POST" class="needs-validation" novalidate
              action="{{ isset($edit_data) ? route('service_category.update', $edit_data->id) : route('service_category.store') }}"
              ...
              <div class="row mb-3 align-items-center">
                ...
                <div class="col-lg-8">
                  <input type="text" class="form-control" placeholder="Plz Fill Slag here" name="slug"
                    value="{{ old('slug', isset($edit_data) ? $edit_data->slug : '') }}">
                  {{-- <span class="required">*</span> --}}
                  @error('slug')
                    <span class="error-msg">{{ $message }}</span>
                  @enderror
              ...
              <div class="form-footer mt-6" style="display: flex; justify-content: flex-end; gap: 10px;">
                <a href="{{ route('service_category.index') }}" class="btn btn-light btn-pill">Cancel</a>
